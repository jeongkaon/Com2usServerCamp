...
미들웨어(Middleware)는 ASP.NET Core 애플리케이션에서 HTTP 요청을 처리하고 응답을 생성하는 구성 요소이다.
미들웨어는 파이프라인 방식으로 연결되어 있으며, 각 미들웨어는 요청을 처리하고 다음 미들웨어로 전달하거나, 요청을 끝까지 처리하여 응답을 생성할 수 있다.

## 미들웨어의 역할

1. **요청 처리**:
   - HTTP 요청을 받아들이고 필요한 작업(인증, 로깅, 예외 처리 등)을 수행한다.
   - 요청을 다음 미들웨어로 전달하거나, 최종 응답을 생성하여 클라이언트에 반환한다.

2. **응답 생성 및 수정**:
   - 미들웨어는 요청을 처리한 후 생성된 응답을 수정할 수 있다.
   - 예를 들어, 응답 헤더를 추가하거나, 특정 조건에 따라 다른 응답을 반환할 수 있다.

## 미들웨어 파이프라인 구성
ASP.NET Core 애플리케이션에서 미들웨어는 `Program.cs` 파일에서 `IApplicationBuilder` 객체를 통해 설정됩니다. 미들웨어는 `Use`, `Run`, `Map` 메서드를 통해 구성된다.

### 1. Use 메서드
`Use` 메서드는 다음 미들웨어로 요청을 전달하는 미들웨어를 구성된된다.
